...
/// 录制音频回调
public protocol RxAuduiRecordDelegate: class {
    /// 录制更新
...
/// 录制音频管理类
public class RxAuduiRecordKit: NSObject {
    /// 单例模式
    public static let shared = RxAuduiRecordKit()
    /// 最小录制时长
    public var minTime: CFTimeInterval = 10
    /// 最长录制时长
    public var maxTime: CFTimeInterval = 60 * 5
    /// 回调协议
    public weak var delegate: RxAuduiRecordDelegate?
    /// 当前录制文件地址
    fileprivate let recordAudioPath = RxLocalFileKit.wavFilePath.path
    ...
    public final func deleteRecordFiles(path: String? = nil) {
        if path == nil {
            RxLocalFileKit.deleteFile(path: self.recordAudioPath)
        } else {
            RxLocalFileKit.deleteFile(path: path!)
        }
    }
}
extension RxAuduiRecordKit: AVAudioRecorderDelegate {
    /// 录制完成回调协议
