...
/// 播放状态枚举
public enum RxMusicPlayerState: Int {
    /// 准备播放
    case Preparing = 1
    /// 开始播放
    case Beigin = 2
...
/// 播放各种状态代理
public protocol RxMusicPlayerDelegate: class {
    /// 更新进度
...
/// 播放状态改变通知
public let RxNotificationNameMusicPlayerStateChange = NSNotification.Name.init(rawValue: "RxNotificationNameMusicPlayerStateChange")
/// 播放状态通知UserInfo - Key
public let RxNotificationMusicPlayerState = "RxNotificationMusicPlayerState"
/// 播放地址通知UserInfo - Key
public let RxNotificationMusicPlayerCurrentPlayUrl = "RxNotificationMusicPlayerCurrentPlayUrl"
/// 播放类型通知UserInfo - Key
public let RxNotificationMusicPlayerType = "RxNotificationMusicPlayerType"
/// 音乐对象
public struct RxModelMusic {
    /// 音乐名称
...
/// 播放方式
public enum RxMusicPlayerType: Int {
    /// 单曲播放 - 不循环
    case single
    /// 列表播放 - 不循环
    case list
...
/// 播放管理器
public class RxMusicPlayerKit: NSObject {
    /// 单例模式
    public static let shared = RxMusicPlayerKit()
    /// 播放器
    ...
    /// 当前播放方式
    public var playType: RxMusicPlayerType = .cycle
    /// 播放速度 改变播放速度
    ...
    /// 音频播放数组
    public var musicArray: [RxModelMusic] = [RxModelMusic]() {
        didSet {
            self.currentIndex = 0
        }
    }
    /// 播放代理
    public weak var delegate: RxMusicPlayerDelegate?
    /// 时间监听器
    private var timeObserVer: Any?
    /// 当前播放对象
    public var currentModel: RxModelMusic?
    /// 初始化播放管理器
    ...
    @objc private func configLockScreenPlay() {
        ...
        if (newTaskID != .invalid) && (self.bgTaskId != .invalid)  {
...
extension RxMusicPlayerKit {
    ...
    private final func reloadAudio(playpath: String) -> URL? {
        if FileManager.default.fileExists(atPath: playpath) {
            return URL.init(fileURLWithPath: playpath)
        }
    ...
    public final func playMusic(model: RxModelMusic, type: RxMusicPlayerType) {
        // 播放前初始化倍速
        self.setPlaySpeed(playSpeed: self.playSpeed)
        // 移除上一首的通知 观察
    ...
    private func updateCurrentPlayState(state: RxMusicPlayerState) {
        NotificationCenter.default.post(name: RxNotificationNameMusicPlayerStateChange,
                                        object: nil,
                                        userInfo: [RxNotificationMusicPlayerState: state,
                                                   RxNotificationMusicPlayerCurrentPlayUrl: self.currentUrl ?? "",
                                                   RxNotificationMusicPlayerType: self.playType])
    }
    /// 格式化时间为字符串
